        ...
        <form className='m-auto my-16 max-w-[1000px] bg-purple-200 p-8 border-2 shadow-md rounded-md'>
          {loading ? (
            ...
            <>
              ...
              <button className='bg-purple-500 text-white px-4 py-2 font-medium hover:bg-purple-600' onClick={handleSubmit}>{mode === "add" ? "Add task" : "Update Task"}</button>
              <button className='ml-4 bg-red-300 text-white px-4 py-2 font-medium' onClick={() => navigate("/")}>Cancel</button>
              {mode === "update" && <button className='ml-4 bg-purple-500 text-white px-4 py-2 font-medium hover:bg-purple-600' onClick={handleReset}>Reset</button>}
            </>
          )}
        </form>
      </MainLayout>
