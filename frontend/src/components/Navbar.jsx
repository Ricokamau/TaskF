...
const Navbar = () => {
  ...
  return (
    <>
      <header className='flex justify-between sticky top-0 p-4 bg-purple-200 shadow-sm items-center'>
      <h2 className='cursor-pointer uppercase font-medium'>
  <Link to="/">
    <img src="/images/logo.png" alt="TaskFusion Icon" className="h-18 w-18 inline-block" />
  </Link>
</h2>
        <ul className='hidden md:flex gap-4 uppercase font-medium'>
          {authState.isLoggedIn ? (
            <>
              <li className="bg-purple-500 text-white hover:bg-purple-600 font-medium rounded-md">
            ...
            <li className='py-2 px-3 cursor-pointer text-purple-500 hover:bg-gray-100 transition rounded-sm'><Link to="/login">Login</Link></li>
          )}
        </ul>
